...
P31 / #7 Peperomia Bicolor and P32 / #8 Tricolor oyster plant retain their assignments. P33/P34 remain retired. The collection now has **42 active profiles plus one historical profile**. These administrative allocations create no Water, Weigh, Repot, initial wet/dry reference, or completed pot setup. The owner assigned P35 / #9 to the **D'vine Dev selected 4.3-inch Blush Mauve** pot and P36 / #10 to the **Thirtypot selected 4.5-inch speckled-brown** pot. Repots and follow-up photographs remain planned for September 22; see the [setup record](../../docs/setup.md) for the separate houseplant stool plan and current purchase evidence.

The migrated bulk schema appends **P35 weight (g)** and **P36 weight (g)** at **BG:BH**, making **60 physical columns, A:BH / 61 including AppSheet's \_RowNumber**. Preserve all existing A:BF values and ordering. BE:BF remain hidden, noneditable, blank-only retired fields. History stays A:AP and App entries stays A:AH. Compatibility queue reads continue to support reviewed 54-, 56-, and 58-column schemas; unrecognized columns or selected IDs fail visibly rather than dropping input.

The saved AppSheet readback compared all **59 existing column definitions** with the prewrite capture and found **zero differences**. P35/P36 are optional positive Decimal inputs with two decimal digits, `[_THIS] > 0`, the expected display names, and the existing Weigh / Water + weigh visibility rule. Round validation, portrait, and natural label-order expressions match the maintained source after whitespace normalization, including both retired-field blank guards. The empty Bulk Log form rendered exactly **34 blank weight inputs**, ordered P01–P32, P35, P36, then Notes; it was canceled without saving.

The native execution panel showed exactly one five-minute `processQueuedAppSheetEntries` trigger, with 0% errors. Version 98 `doGet` at **4:21:28 p.m. EDT** and bootstrap at **4:21:31 p.m.** completed in **1.557 s / 7.777 s**. Production HEAD queue executions at **4:18:33 p.m. / 4:23:33 p.m.** completed in **4.891 s / 4.861 s**. These are observed execution timings, not performance guarantees.

The deployed model withholds forecast dates/windows and automatic watering dates for **P28, P35, and P36**. Their readiness is manual leaf-cycle inspection; a dry reference, plateau, or wrinkled old leaves alone does not authorize watering. Inspect each Lithops group before combining it. New empty histories display “No watering recorded.” No generic alternating-feed rule is introduced. See the [collection exceptions](../../docs/watering-strategy.md#collection-exceptions).

Artwork revision **b86b51605d0afb26** is published in the private [GardenPlantPortraits-b86b51605d0afb26 folder](https://drive.google.com/drive/folders/1GTm2DLCyUchPhkWB1dNqfRGRQZovzXPJ), with all **34 portrait files verified**. The maintained AppSheet expression targets this revision. Previous folders remain available for historical clients.

**Production workbook verification, September 21 at 20:19 UTC:** a [native backup](https://docs.google.com/spreadsheets/d/1KpysrgYglfl3OavWTM_cdqHubyOlnFIiR8vT8PIbsdc/edit) preceded the guarded migration. The completed workbook has **58 tabs and 164 charts**. The strict readback compared **1,249,687 cells / 4,998,748 fields**, including **534,000 canonical, staging, and RO cells**, with **zero errors or unexplained changes**. All **156 original chart identities** were preserved and eight new charts added. All **19,996 shared-role helper outputs** and **four unplotted sentinels** passed their checks.

Canonical History remained **1,094 records with 1,094 unique observation IDs and 993 request-ID groups**, with identical before/after entered-record digests. Shared request IDs legitimately group multiple event rows. Both new plant histories remain empty, with manual leaf-cycle guidance and no invented observation or baseline. The retained private production proofs are `.cache/mesemb-production-20260921/production-workbook-receipt.json` and `production-verification.json`.

Website/source commit **12921cdc** passed the required checks, SonarCloud, and Pages publication. The public site retained **93 available collection previews and the same two known unavailable captures**. The existing daily task was updated and verified while retaining its **9:45 a.m. America/New_York** schedule and the new identity/leaf-cycle exceptions. No synthetic observation was submitted to production.

Final source validation passed **993 logger tests** with 99.48% statement, 98.22% branch, 100% function, and 99.71% line coverage, preserving the 90% floors. Full typechecking and the website build/check passed with 43 profiles and 34 pot allocations. The production readback above supplies separate live-state evidence.

**Native-copy rehearsal, September 21:** the [disposable workbook](https://docs.google.com/spreadsheets/d/1veKWNBOeSW40GlJTwDWeD3tq-vbbB2qlt9iYfT9ZBG4/edit) has 58 tabs and 164 charts, with P35/P36 pages at sheet IDs **202609350 / 202609360**. The final strict comparison passed **1,001,413 cells / 4,005,652 fields**, including **324,000 canonical, staging, and RO cells**, with no unexplained differences or formula errors. All **156 original chart identities and positions** remain intact, with only the reviewed inventory/specification changes, plus eight new charts. The native rehearsal workbook remains available, and the fresh production proofs above are retained separately. The earlier local rehearsal cache is no longer present.
...
The copy's bound source and calculated model cells exercised the new roster and manual leaf-cycle output. A separate no-write verification helper was uploaded but **not executed** because the copied project requested a new authorization grant; no new grant was accepted during that rehearsal. The later production rollout is documented above.
...
This dated rollout is historical; the September 21 enrollment above supplies the current roster, versions, and schema width.
