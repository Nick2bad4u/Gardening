...
Edit that document to update the September 15 four-table arrangement: six columns by four rows of small pots at the front, five larger/shared containers on the rear wooden tables and #3 Money Tree separately on the north windowsill, confirmed September 16, the AW200 + AeroLight 240 W operating record, exposure priorities and evidence. Then run `npm run build:booklet`. The seven placement diagrams open at full size, and plant links navigate to the corresponding profiles. The blank phone-reading CSV is copied into the published site. Coordinates and nominal footprint are photo-matched or inferred; the source document remains readable in the repository.

The placement page includes [four illustrated views](../layouts/table-placement-research.md#illustrated-views) added September 16: whole display, front pots, rear containers/windowsill, and relative-light hypothesis, followed by the three original arrangement diagrams. Only the illustrated relative-light map appears inline; the original reference map and its transparent-background copy remain available as downloads. Image prompts and evidence limits are stored with the new assets.
