...
Edit that document to update the September 15 four-table arrangement: six columns by four rows of small pots at the front, six larger/shared containers on the rear wooden tables, the AW200 + AeroLight 240 W operating record, exposure priorities and evidence. Then run `npm run build:booklet`. The estimated relative-light map and three arrangement diagrams open at full size, and plant links navigate to the corresponding profiles. The blank phone-reading CSV is copied into the published site. Coordinates and nominal footprint are photo-matched or inferred; the source document remains readable in the repository.
