...
P33 needs partial drying without prolonged drought; P34 needs an upper **1–2-inch** dryness check scaled to its actual root-ball depth once measured. Keep both as manual-readiness exceptions: weight curves and cactus plateaus do not prove their root-zone condition. Enrollment supplies no synthetic observations, watering, repot, measured pot size, medium, or physical setup. The owner confirms two Amazon Basics eight-inch pots with drainage holes and plans to place them on floor stools, off the tables, with a greenhouse-mix/Molly's blend; proportions and completed moves/repots are not yet recorded. Both current nursery pots are 6 inches and described by the owner as quite full; depth, drainage, and existing medium remain unrecorded. The reviewed plant-tab IDs are **202609330** for P33 and **202609340** for P34.
...
**Native enrollment and logger deployment are verified:** production now has 56 tabs and 156 charts, retaining all 148 earlier chart identities and all 1,075 History records, 38 App entries, empty bulk staging, and RO data. The new pages have four charts and warning protections. Logger **5.27.0 / immutable version 96** uses the existing phone URL; its three deployed source files match the maintained source. Both public plant profiles and their evidence/portrait assets are live. The [logger enrollment runbook](../scripts/google-sheets/README.md#september-20-purchased-houseplant-enrollment) records the complete preservation audit.

**Production AppSheet version 1.100109 is saved and verified after an editor reload.** The 59-column schema retains all 57 previous column settings. P33/P34 are optional positive Decimal weights with the existing Weigh / Water + weigh visibility rule, positioned after P30 and before Notes in Bulk Log. The Round action uses the maintained 32-weight validation and both archived blank guards. Verification uses empty forms and read-only views without creating a test observation.
...
The September 5 artwork contained 38 exported SVGs, including separate portraits
for the shared P19 and P20 planters. That AppSheet release used thirty P01-P30 portraits
...
update the checked-in manifest, upload all mapped current Pxx SVGs to a new
revisioned folder beside the workbook, and update the AppSheet expression.
Verify every current roster image after saving and syncing. Keep older folders until
