...
| `#7` / `P31`, new four-succulent planter  | Start with plain RO at an otherwise-ready watering while establishment and nursery fertilizer history are unknown. After healthy active growth and the shared dry-down are documented, use the existing shared-succulent rule above. Four profiles do not create four doses; no feed is due merely because the inventory is active.     |
| `#8` / `P32`, Nanouk tradescantia         | Keep separate from cactus feed/plain alternation and dry-down timing. Use plain water while establishment or previous fertilizer is unknown. No collection-specific MSU dose has been established for this new plant; do not silently copy the money-tree rate. A manual Check-only report entry has no automatic water or feed recipe. |
