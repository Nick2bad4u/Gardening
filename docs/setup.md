...
The cactus display uses continuous rotating canopy airflow and two Honeywell fans below the perforated tables, both on low. One is the HT900; the older, larger model is unidentified. The A22 controls fan cycles, with timings unrecorded. The TABYIK dehumidifier runs directly from a wall outlet.

P31 and P32 remain in six-inch nursery pots at their last recorded location on the room-side floor near the money tree. Their eight-inch repots and living-room display are planned below. The proposed medium combines airy, perlite-containing greenhouse mix with Molly's Succulent Mix; the ratio is undecided. Check root-ball fit, drainage, and moisture before repotting or watering.
...
The **September 22 plan** places **P31 / #7 Peperomia Bicolor and P32 / #8 Tricolor oyster plant** side by side at the **north-facing double glass sliding door**, with **P32 closest to the glass**, **P31 on the smallest Bamworld stool**, and the **FECiDA 54 W six-tube light clipped to a shelf**. Two Juhefa three-head fixtures are available, each seller-rated about 10 W total; the optional Diivoo's input wattage is unverified. See the [equipment inventory](./equipment/inventory.md#lights-and-controls) for exact models.

The last recorded location is the room-side floor near the money tree on September 20. Repots, the move, stool placement, and light installation remain unconfirmed. The living-room destination is separate from P21's cactus-room windowsill; clearances, exposure, and room conditions still need measurement.
...
The current diagram faces the window: **north window top, open room bottom, Mylar left, white wall right**. This rotates the old schematic view without changing the room. The two-foot end reflector remains absent. See the [current placement guide](./layouts/table-placement-research.md) for the current positions, earlier illustrative light map and [owner light readings](./layouts/table-placement-research.md#owner-light-readings-and-confirmed-swap). The 48 approximate lux-derived PPFD readings were taken at individual plant heights, not one level plane. The whole mapped rectangle, including the intentionally unlit far edge, ranged from 145 to 677 estimated PPFD; the app's 13-hour DLI output ranged from 6.8 to 31.7. This is not calibrated PAR evidence, and the measurement date was not supplied. Only the A3/F1 exchange was confirmed after review. Old proposed swaps and the glass-spill gradient are historical.
...
**Room observations, September 20:** the owner reports approximately **45–50% RH** and temperatures **in the 70s °F** near the collection. These are approximate current conditions, not a recorded day/night minimum or maximum. Three sensing points are reported: a probe on the **far pair of wooden tables**; a probe at plant height on the wall with about two inches of free lead allowing it to lean away from the wall; and an internal sensor in a body on a shelf about two feet from the cactus table. The equipment includes the probe-equipped VIVOSUN app-connected sensor and the A22 probe; the exact probe-to-device assignment is unconfirmed. The table probe is on the wooden display; the glass table is retired.
...
The care history records the first setup-2 plain Beauchamp's RO cycle for
...
| Mountain Crest received/current pots | Received: six 2 in nursery pots; current: six matching 4 in round pots measuring 4 × 4 × 3.7 in                                     | Received-pot shapes are not reliably documented. The owner reports removing about 95% of the nursery soil during the 2026-08-28 repot. Recorded pot size is 4 in for all six. September 20 medium record: Molly's Succulent Mix with approximately 20% added perlite (roughly 80/20), recalled rather than measured; this describes the existing repot, not a new setup.                                                                                                                   |
...
removing about 95% of the nursery soil during repotting. The recorded
pot size is 4 inches for all six. They now share the same round
4 × 4 × 3.7-inch pot model.

The permanent mapping is `P23`/`G2` paper spine, `P24`/`H1` Coconut
Crystal, `P25`/`H2` Raindrops, `P26`/`H3` Eve's needle, `P27`/`G1` Black Widow,
and `P28`/`G3` Royal Flush. The August 29, 2026 measurements were:
...
six. It does not document watering or fertilizer use. The owner confirmed the arrangement in the [current placement guide](./layouts/table-placement-research.md), including the later A3/F1 exchange. Exact individual plant-to-light clearance remains unmeasured. On September 20 the owner clarified that all six use Molly's with approximately 20% added perlite, probably less than the 40% used for P01–P18. The approximately 80/20 recipe is an estimate of the existing mix.
...
Use current moisture checks and pot measurements when assessing these containers. P30 has one shared history for its aggregate overview and three component groups (Succulent-10A/B/C). Weigh and water it once as a complete pot; separate component profiles describe the plants within it.
...
The starting pot records retain these substrate and pot descriptions. They
...
- precise measured pot geometry beyond the logged P29/P30 size descriptions and any still-needed plant dimensions; their setup-2 repot media are recorded in the Repot observations, placement is confirmed, and current weights must be read from the care history;
