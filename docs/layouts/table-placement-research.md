...
On **September 21**, the owner plans to move **P31 and P32 to a north-facing window in the living room** and subsequently confirmed purchasing a **FECiDA advertised 54 W six-tube clip grow light** to make their lighting more even and stronger. The owner explicitly plans to run it **together with both existing 10 W LED bars and the separate 10 W LED light**. Amazon identifies [ASIN B0DHRHF2QV](https://www.amazon.com/dp/B0DHRHF2QV), with arrival estimated for **Wednesday, September 23**. The earlier 20 W three-head alternatives were only considered. The [equipment record](../equipment/inventory.md#lights-and-controls) separates the confirmed purchase from seller specifications. This is a separate destination from P21 Money Tree's recorded windowsill in the cactus room. Receipt, the move, mounting, stool assignments, timer settings, distances, actual power draw, and leaf-level coverage remain unconfirmed or unmeasured.
...
1. Measure **all four planned fixtures operating together after dark** at representative upper, outer, and shaded leaf positions on both plants, recording distances, control settings, and runtime. The owned **UNI-T UT383BT with Android PPFD Meter** can provide useful estimates; retain raw lux and the app's spectrum setting, because conversion for the combined, partly unknown spectra is not a calibrated PAR measurement.
...
4. Adjust spacing and aim from the **combined readings**, using dimming only where a fixture actually supports it. Avoid concentrating all the beams on one small central patch. Individual-fixture readings are optional troubleshooting comparisons, not a replacement for checking the intended combined setup. Acclimate gradually and watch for stretching, weaker color, bleaching, or scorch before considering any further purchase.
...
- **September 21 owner evidence:** planned P31/P32 move to a living-room north-facing window; two 10 W LED bars and another 10 W light already owned. The later message and screenshot confirm the FECiDA six-tube purchase, and the follow-up explicitly confirms intended use with all three existing lights. Receipt, installation, distances, and applied settings remain unconfirmed.
