...
| Latest pocket report            | `/Gardening/pocket-report/`         |
...
The publication builder downloads only selected Gyazo thumbnails actually used on public pages. It caches captures under `.cache/collection-previews-v1` and publishes metadata-stripped WebP previews at 320, 640, and 960 pixels, capped at source width. Smaller images do not create duplicate variants. Licensed reference photographs and rendered local nursery-evidence photos receive responsive publication variants at 480, 960, and 1440 pixels, capped at source width. Attribution and source links remain available; full-size local nursery-evidence files are retained for download links and existing logger URLs.
